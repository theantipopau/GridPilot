...
const CAPABILITY_LABEL: Record<string, string> = {
  ELIGIBLE: "✓ confirmed qualified",
  REVIEW_REQUIRED: "qualification not yet reviewed",
};

export default function SuggestionCandidateCard({ candidate: c, onApply, applying }: Props) {
  const teacherChanged = c.before.teacher_code !== c.after.teacher_code;
  return (
    <div className="rounded bg-white px-2 py-1.5 text-xs">
      <div className="flex items-center justify-between gap-2">
        <span>
          ...
          {teacherChanged ? (
            <>
              {c.before.teacher_code}
              <span className="mx-1 opacity-50">→</span>
              {c.after.teacher_code}
              <span className="ml-1 opacity-50">
                ({c.before.day_code} {c.before.period_code} {c.before.room_code})
              </span>
            </>
          ) : (
            <>
      ...
      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] opacity-60">
        ...
        {c.why && teacherChanged && c.why.capability_status && (
          <span>{CAPABILITY_LABEL[c.why.capability_status] ?? c.why.capability_status}</span>
        )}
        ...
        {c.class_teacher_familiarity && (
          <span>
            {c.class_teacher_familiarity.same_teacher_elsewhere_count > 0
              ? `this class already has this teacher ${c.class_teacher_familiarity.same_teacher_elsewhere_count}/${c.class_teacher_familiarity.total_other_lessons} other lessons`
